...
import { okImageSource } from "./resolve-image.js";
...
export const unresolvedNodes = (nodes) =>
  [...new Set(nodes.filter((n) => !SERVICES[n.id]?.icon && !okCustomIcon(n.icon) && !okImageSource(n.image)).map((n) => n.id))];
...
export function validateDesign({ nodes, edges = [] }) {
  ...
  for (const n of nodes) {
    if (n.image != null && !okImageSource(n.image)) {
      return { error: `Node "${n.id}" has an image that is not a usable source. Give a data:image/(png|jpeg|webp|gif);base64 URI, an https image URL, an absolute file path (MCP only), or airclips:<id> / airclips:latest.` };
    }
  }
  const unresolved = unresolvedNodes(nodes);
  if (unresolved.length) {
    return {
      error: `Every node must render a real logo: use a known catalog service id, OR give the node a custom "icon" (a remote https URL, a data:image/... URI, or a same-origin path like "/brand/foo.svg") plus a "label", OR make it a picture node with "image". Unresolved: ${unresolved.join(", ")}.`,
      unresolved,
    };
  }
