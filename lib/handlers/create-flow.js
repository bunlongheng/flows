...
export default async function createFlow(req, res) {
  ...
  const out = {
    ...
    // The animated embed for a README. w=1600 is 2x the 880px GitHub renders
    // a README at, so it stays sharp on a retina screen.
    gif_url: `${APP_URL}/api/flows/${encodeURIComponent(slug)}?format=gif&w=1600`,
    readme: `![${title}](${APP_URL}/api/flows/${encodeURIComponent(slug)}?format=gif&w=1600)`,
  };
