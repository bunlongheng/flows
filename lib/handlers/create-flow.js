...
function bad(res, error, extra = {}) {
  return res.status(400).json({
    error,
    supported_type: "flows ONLY (React Flow { nodes, edges } structure)",
    required_fields: {
...
export default async function createFlow(req, res) {
  ...
  const { title, nodes, edges = [], type = "flows" } = body;
  ...
  if (type && type !== "flows") return bad(res, `Unsupported type: "${type}".`);
  ...
  const { rows } = await db.query(
    ...
    [owner, title.trim(), slug, JSON.stringify(normalizedNodes), JSON.stringify(edges), type, ["API"], isPublic, pattern, description],
  );
