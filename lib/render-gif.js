...
// as motion without turning a README into a download. ?w= goes up to 3200 for
// a README on a retina screen: 2.4s and just under 1MB on a laptop, so the
// route carries its own time budget for a cold render.
...
export function renderDiagramGif(nodes, edges, opts = {}) {
  ...
  const width = Math.max(200, Math.min(3200, opts.width || GIF_WIDTH));
